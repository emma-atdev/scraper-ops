...
- 실패 보고서 생성
- Slack 알림 (정책: 변화 발생·실패 시 매시간 + 매일 09:00 요약. 정상 + 변화 없음은 조용)
...
운영 채널 노이즈 최소화를 위해 다음 정책을 둔다.

- `failure_or_change` (기본): 실패 또는 신규/변경 데이터 발생 시에만 매 run 알림
- `failure`: 실패 시에만
- `always`: 매 run 발송 (디버깅용)
- `never`: 무음
- 매일 09:00 일일 요약을 별도 발송한다 (직전 24시간 site별 실행 횟수·실패·신규·변경 통계)
- 메시지 본문은 한국어로 작성한다 (제목·라벨·진단 카테고리). issue code, run_id 등 식별자는 원문 유지.

### 메시지 구조 (M6 self-healing 단계)
